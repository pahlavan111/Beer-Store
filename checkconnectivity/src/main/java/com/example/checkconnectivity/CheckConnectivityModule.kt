...
object CheckConnectivityModule {
    ...
    fun initialize(context: Context) {
        ...
        connectionLiveData.observeForever { hasConnection ->
            Log.d("CheckConnectivityModule", "hasConnection :  $hasConnection")
            this.hasConnection = hasConnection

            coroutineScope.launch {
                hasInternet = if (hasConnection) {
    ...
    private suspend fun internetConnectionAvailable(timeOut: Int): Boolean {
        return try {
            withTimeout(timeOut.toLong()) {
                try {
                    val inetAddress = InetAddress.getByName("radar.arvancloud.ir")
                    inetAddress.hostAddress?.isNotEmpty() == true
